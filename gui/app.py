...
from slideshow.SlideManager import ImageSlide
from slideshow.SlideManager import SlideManager
from slideshow.SlideManager import VideoSlide
...
class App(tk.Tk):
    def __init__(self, title="", *args, **kwargs):
        ...
        self.addSlideButton = ttk.Button(
            master_frame, text="Add slide", command=self.addSlide, state=tk.DISABLED
        )
        ...
        # Image Frame with fill parent (sticky=tk.NSEW)
        self.addAudioButton = ttk.Button(
            master_frame, text="Add audio", command=self.addAudio, state=tk.DISABLED
        )
        ...
        labelVideoDurationValue = tk.Label(
            frameActions, textvariable=self.videoDurationValue
        )
        ...
        labelAudioDuration = tk.Label(
            frameActions, textvariable=self.audioDurationValue
        )
        ...
        moveButton = ttk.Checkbutton(
            frameActions, text="Move slides/audio", variable=self.moveSlidesBtn
        )
        moveButton.grid(row=0, column=3, rowspan=2, sticky=tk.W, padx=2)

        buttonSync = tk.Button(
            frameActions, text="Sync Video to Audio", command=self.syncToAudio
        )
        buttonSync.grid(row=0, column=4, rowspan=2, sticky=tk.W, padx=2)

        buttonResetTime = tk.Button(
            frameActions, text="Reset slide durations", command=self.resetSlideDurations
        )
        buttonResetTime.grid(row=0, column=5, rowspan=2, sticky=tk.W, padx=2)

        buttonCreateVideo = tk.Button(
            frameActions, text="Create Video", command=self.createVideo
        )
        ...
        self.filemenu.add_command(
            label="Save", command=self.saveSlideshow, state="disabled"
        )
        self.filemenu.add_command(
            label="Save As..", command=self.saveAs, state="disabled"
        )
        ...
        self.generalmenu.add_command(
            label="Slideshow Settings",
            command=self.slideshowSettingsWindow,
            state="disabled",
        )
        self.generalmenu.add_separator()
        self.generalmenu.add_command(
            label="General Settings", command=self.generalSettingsWindow
        )
        ...
        self.style.map(
            SUNKABLE_BUTTON,
            background=[("pressed", "red"), ("disabled", "red"), ("focus", "red")],
        )
    ...
    def hasSlides(self):
        return self.sm and (
            len(self.sm.getSlides()) > 0 or len(self.sm.getBackgroundTracks()) > 0
        )
    ...
    def onCloseSlideshowSettings(self, toplevel):
        input_files = [
            slide.getObject(self.slideshow_config) for slide in self.sm.getSlides()
        ]
        ...
        toplevel.destroy()
        # ask for reload
        if self.hasSlides() and messagebox.askyesno(
            "Restart",
    ...
    def slideshowSettingsWindow(self):
        ...
        choices = {
            "zoom_direction_x": self.zoom_direction_choices_x,
        ...
        filewin.protocol(
            "WM_DELETE_WINDOW", (lambda: self.onCloseSlideshowSettings(filewin))
        )
    ...
    def checkEntryModification(self, event):
        ...
        photo = self.getPreviewImage(
            self.buttons[self.slide_selected].image_path, zd_x, zd_y, zd_z, zr, sc
        )
    ...
    def saveSlide(self):
        if self.slide_changed:
            ...
            if isinstance(slide, VideoSlide):
                ...
                slide.start = (
                    float(self.inputvideoStart.get())
                    if float(self.inputvideoStart.get()) > 0
                    else None
                )
                slide.end = (
                    float(self.inputvideoEnd.get())
                    if (
                        float(self.inputvideoEnd.get()) > 0
                        and float(self.inputvideoEnd.get()) < slide.getDuration()
                    )
                    else None
                )
            ...
            slide.title = (
                self.inputSubtitle.get() if len(self.inputSubtitle.get()) > 0 else None
            )

            if (
                len(self.inputOverlayTextTitle.get()) > 0
                ...
                if len(self.inputOverlayTextFontSize.get()) > 0:
                    overlay_text["font_size"] = float(
                        self.inputOverlayTextFontSize.get()
                    )
                if len(self.inputOverlayTextDuration.get()) > 0:
                    overlay_text["duration"] = float(
                        self.inputOverlayTextDuration.get()
                    )
                ...
                if len(self.inputOverlayTextTransitionX.get()) > 0:
                    overlay_text[
                        "transition_x"
                    ] = self.inputOverlayTextTransitionX.get()
                if len(self.inputOverlayTextTransitionY.get()) > 0:
                    overlay_text[
                        "transition_y"
                    ] = self.inputOverlayTextTransitionY.get()

                slide.overlay_text = overlay_text if overlay_text else None

            if (
                len(self.inputOverlayColorColor.get()) > 0
                ...
                if len(self.inputOverlayColorOpacity.get()) > 0:
                    overlay_color["opacity"] = float(
                        self.inputOverlayColorOpacity.get()
                    )
                if len(self.inputOverlayColorDuration.get()) > 0:
                    overlay_color["duration"] = float(
                        self.inputOverlayColorDuration.get()
                    )
    ...
    def newSlideshow(self):
        if self.hasSlides() and messagebox.askyesno(
            "New", "Do you want to save the current slideshow?"
        ):
            self.saveSlideshow()
        self.init()
        self.createSlideshow()
    ...
    def openFile(self):
        ...
        try:
            ...
            if res:
                self.title(f"{self.general_title} ({self.filename})")
    ...
    def createSlideshow(self, input_files=[], audio_files=[]):
        ...
        try:
            ...
            messagebox.showerror(
                "Error",
                "Could not initiate Slideshow Manager.\nPlease check general Settings.\n\nError: %s"
                % (e),
            )
            return False
        ...
        for i, slide in enumerate(self.sm.getSlides()):
            ...
            if isinstance(slide, VideoSlide):
                img_path = os.path.join(
                    self.sm.tempFileFolder, "thumbnail_%s.jpg" % (i)
                )
                si = None
    ...
    def onSlideClicked(self, button_id):
        ...
        buttonDeleteSlide = tk.Button(
            buttonsFrame, text="Remove", command=(lambda: self.deleteSlide())
        )
        ...
        transitionCombo = ttk.Combobox(
            transitionframe,
            values=self.transition_choices,
            width=30,
        ...
        durationFadeEntry = tk.Entry(
            transitionframe,
            validate="all",
            validatecommand=(vcmd, "%P"),
            textvariable=self.inputDurationTransition,
        )
        ...
        if isinstance(slide, ImageSlide):
            ...
            durationMinEntry = tk.Entry(
                imageframe,
                validate="all",
                validatecommand=(vcmd, "%P"),
                textvariable=self.inputDurationMin,
            )
            ...
            zoomDirectionXCombo = ttk.Combobox(
                imageframe,
                values=self.zoom_direction_choices_x,
                textvariable=self.inputZoomDirectionX,
            )
            zoomDirectionXCombo.grid(row=2, column=1, sticky=tk.E, padx=4, pady=4)
            zoomDirectionXCombo.bind(
                "<<ComboboxSelected>>", self.checkEntryModification
            )
            ...
            zoomRateEntry = tk.Entry(
                imageframe,
                validate="all",
                validatecommand=(vcmd, "%P"),
                textvariable=self.inputZoomRate,
            )
            ...
            scaleModeCombo = ttk.Combobox(
                imageframe,
                values=self.scale_mode_choices,
                textvariable=self.inputScaleMode,
            )
        ...
        durationEntry = tk.Entry(
            ...
            validate="all",
            validatecommand=(vcmd, "%P"),
            textvariable=self.inputDuration,
        )
        ...
        subtitleEntry = tk.Entry(
            subtitleFrame, textvariable=self.inputSubtitle, width=50
        )
        ...
        self.inputOverlayColorDuration.set(
            ...
            if slide.overlay_color and "duration" in slide.overlay_color
            else ""
        )
        ...
        overlayColorDurationEntry = tk.Entry(
            overlayFrameColor,
            validate="all",
            validatecommand=(vcmd, "%P"),
            textvariable=self.inputOverlayColorDuration,
        )
        ...
        self.inputOverlayColorOffset.set(
            ...
            if slide.overlay_color and "offset" in slide.overlay_color
            else 0
        )
        ...
        overlayColoOffsetEntry = tk.Entry(
            overlayFrameColor,
            validate="all",
            validatecommand=(vcmd, "%P"),
            textvariable=self.inputOverlayColorOffset,
        )
        ...
        self.inputOverlayColorColor.set(
            ...
            if slide.overlay_color and "color" in slide.overlay_color
            else "black"
        )
        ...
        overlayColorColorEntry = tk.Entry(
            overlayFrameColor, textvariable=self.inputOverlayColorColor, width=50
        )
        ...
        self.inputOverlayColorOpacity.set(
            ...
            if slide.overlay_color and "opacity" in slide.overlay_color
            else 0.8
        )
        ...
        overlayColorOpacityEntry = tk.Entry(
            overlayFrameColor, textvariable=self.inputOverlayColorOpacity, width=50
        )
        ...
        self.inputOverlayTextTitle.set(
            slide.overlay_text["title"]
            if slide.overlay_text and "title" in slide.overlay_text
            else ""
        )
        ...
        self.inputOverlayTextFont.set(
            slide.overlay_text["font"]
            if slide.overlay_text and "font" in slide.overlay_text
            else ""
        )
        overlayFontLabel = tk.Label(overlayFrameText, text="Font")
        overlayFontLabel.grid(row=1, column=0, sticky=tk.W, padx=4, pady=4)
        overlayFontEntry = tk.Entry(
            overlayFrameText, textvariable=self.inputOverlayTextFont, width=50
        )
        ...
        self.inputOverlayTextFontFile.set(
            slide.overlay_text["font_file"]
            if slide.overlay_text and "font_file" in slide.overlay_text
            else ""
        )
        ...
        overlayFontFileEntry = tk.Entry(
            overlayFrameText, textvariable=self.inputOverlayTextFontFile, width=50
        )
        ...
        self.inputOverlayTextFontSize.set(
            ...
            if slide.overlay_text and "font_size" in slide.overlay_text
            else ""
        )
        ...
        overlayFontSizeEntry = tk.Entry(
            overlayFrameText,
            validate="all",
            validatecommand=(vcmd, "%P"),
            textvariable=self.inputOverlayTextFontSize,
        )
        ...
        self.inputOverlayTextColor.set(
            ...
            if slide.overlay_text and "color" in slide.overlay_text
            else "white"
        )
        ...
        overlayFontColorEntry = tk.Entry(
            overlayFrameText, textvariable=self.inputOverlayTextColor, width=50
        )
        ...
        self.inputOverlayTextDuration.set(
            ...
            if slide.overlay_text and "duration" in slide.overlay_text
            else ""
        )
        ...
        overlayFontDurationEntry = tk.Entry(
            overlayFrameText,
            validate="all",
            validatecommand=(vcmd, "%P"),
            textvariable=self.inputOverlayTextDuration,
        )
        ...
        self.inputOverlayTextOffset.set(
            ...
            if slide.overlay_text and "offset" in slide.overlay_text
            else 0
        )
        ...
        overlayFontOffsetEntry = tk.Entry(
            overlayFrameText,
            validate="all",
            validatecommand=(vcmd, "%P"),
            textvariable=self.inputOverlayTextOffset,
        )
        ...
        self.inputOverlayTextTransitionX.set(
            ...
            if slide.overlay_text and "transition_x" in slide.overlay_text
            else "center"
        )
        overlayTransitionXLabel = tk.Label(
            overlayFrameText, text="Transition X Direction"
        )
        overlayTransitionXLabel.grid(row=7, column=0, sticky=tk.W, padx=4, pady=4)
        overlayTransitionXCombo = ttk.Combobox(
            overlayFrameText,
            values=["center", "right-to-center", "left-to-center"],
            textvariable=self.inputOverlayTextTransitionX,
        )
        overlayTransitionXCombo.grid(row=7, column=1, sticky=tk.W, padx=4, pady=4)
        overlayTransitionXCombo.bind(
            "<<ComboboxSelected>>", self.checkEntryModification
        )

        self.inputOverlayTextTransitionY.set(
            slide.overlay_text["transition_y"]
            if slide.overlay_text and "transition_y" in slide.overlay_text
            else "center"
        )
        overlayTransitionYLabel = tk.Label(
            overlayFrameText, text="Transition Y Direction"
        )
        overlayTransitionYLabel.grid(row=8, column=0, sticky=tk.W, padx=4, pady=4)
        overlayTransitionYCombo = ttk.Combobox(
            overlayFrameText,
            values=["center", "top-to-bottom", "bottom-to-top"],
            textvariable=self.inputOverlayTextTransitionY,
        )
        overlayTransitionYCombo.grid(row=8, column=1, sticky=tk.W, padx=4, pady=4)
        overlayTransitionYCombo.bind(
            "<<ComboboxSelected>>", self.checkEntryModification
        )
        ...
        photo = self.getPreviewImage(
            self.buttons[button_id].image_path, zd_x, zd_y, zd_z, zr, sc
        )
    ...
    def getPreviewImage(
        ...
        output_ratio = float(self.slideshow_config["output_width"]) / float(
            self.slideshow_config["output_height"]
        )
        thumb_width = 250
        ...
        if scale == "crop_center":
            if slideImage_ratio < output_ratio:
                slidethumb_width, slidethumb_height = [
                    thumb_width,
                    int(thumb_width / slideImage_ratio),
                ]
            else:
                slidethumb_width, slidethumb_height = [
                    int(thumb_height * slideImage_ratio),
                    thumb_height,
                ]
        ...
        # transition preview
        if (
            zoom_direction_x is not None
            and zoom_direction_y is not None
            and zoom_direction_z is not None
            and zoom_direction_z != "none"
            and zoom_rate is not None
        ):
            ...
            if scale == "pan":
                z_initial = (
                    slideImage_ratio / output_ratio
                    if slideImage_ratio > output_ratio
                    else output_ratio / slideImage_ratio
                )
            ...
            if scale == "pad" or scale == "crop_center":
                ...
                if (
                    zoom_direction_y == "center" and slideImage_ratio < output_ratio
                ) or (
                    zoom_direction_x == "center" and not slideImage_ratio < output_ratio
                ):
                    top_left = (x0_1, y0_1)
                    top_right = (x0_2, y0_1)
        ...
        if scale == "pan":
            img = img.crop(
                (
                    thumb_x,
                    thumb_y,
                    thumb_x + slidethumb_width,
                    thumb_y + slidethumb_height,
                )
            )
    ...
    def loadSlideshowAudioRow(self):
        ...
        for i, audio in enumerate(self.sm.getBackgroundTracks()):
            ...
            b = ttk.Button(
                frame, text=os.path.basename(audio.file), style=SUNKABLE_BUTTON
            )
            b.grid(row=0, column=i, sticky=tk.NSEW)
    ...
    def onAudioClicked(self, button_id):
        ...
        buttonDeleteAudio = tk.Button(
            buttonsFrame, text="Remove", command=(lambda: self.deleteAudio())
        )
    ...
    def createVideo(self):
        ...
        if filename:
            createVideoThread = threading.Thread(
                target=self.startVideoCreation, args=(filename,), daemon=True
            )
    ...
    def startVideoCreation(self, output_file):
        (
            ...
            inputs,
            temp_filter_script,
        ) = self.sm.prepareVideoProcessing(output_file)

        queue_length = self.sm.queue.getQueueLength()
        frames = self.sm.getFinalVideoFrames()

        progressPopup = ProgressFrame(self)
        progressPopup.create(
            self.slideshow_config["generate_temp"], queue_length, frames
        )
        ...
        for idx, item in enumerate(self.sm.queue.getQueue()):
            if progressPopup.is_cancelled:
                ...
                break
            print(f"Processing video {idx + 1}/{queue_length}")
            logger.info(f"Processing video {idx + 1}/{queue_length}")
            # logger.debug(item)
            tempFile = self.sm.queue.createTemporaryVideo(
                self.slideshow_config["ffmpeg"], item
            )
    ...
    def addSlide(self):
        ...
        filetypes = [
            ".%s" % type
            for type in self.slideshow_config["IMAGE_EXTENSIONS"]
            + self.slideshow_config["VIDEO_EXTENSIONS"]
        ]

        ftypes = [("Slide Files", " ".join(filetypes))]
    ...
    def addAudio(self):
        ...
        ftypes = [("Audio Files", " ".join(filetypes))]
